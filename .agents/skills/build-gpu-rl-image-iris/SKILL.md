...
`wheel-builder` is the script and Dockerfile default, so explicitly set
`WHEEL_SOURCE=prebuilt-wheelhouse` and `PREBUILT_WHEEL_ARTIFACT_URI` to take the
fast path. Use `WHEEL_SOURCE=wheel-builder` only when a multi-hour source rebuild
is explicitly intended. To publish a wheel artifact without pushing an image, set
`WHEEL_ARTIFACT_URI`; that mode builds only `wheel-builder`, extracts `/wheels`,
uploads the archive, and does not require GHCR credentials.
...
WHEEL_URI="s3://bucket/${SCRATCH_PREFIX}-wheel-artifacts/grug-vllm-wheels.tar.gz"
