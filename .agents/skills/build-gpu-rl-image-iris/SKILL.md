...
on any mismatch or fetch failure. The Dockerfile checks the manifest again before
installing the wheels. This path never falls through to a source compile.
...
The resulting image is validated for Grug FSDP2 scratch runs. Do not promote it
to `DEFAULT_RL_DOCKER_IMAGE` until the Marin vLLM dependency closure and the old
fork's `SKYRL_FUSE_WEIGHTS` behavior are reconciled and the retained non-Grug
GPU-RL gates pass.
