...
def weight_sync_requires_cuda(model_type: str, name: str) -> bool:
    ...
    FSDP2 can leave persistent buffers on CPU even while parameters are sharded
    on CUDA. Both supported Grug transports (NCCL broadcast and CUDA IPC) need
    its replicated query-bias buffer on a CUDA device before transfer.
    """
