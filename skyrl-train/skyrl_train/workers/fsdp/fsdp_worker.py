    ...
    def extract_weights(self, dtype: torch.dtype):
        ...
        if not self.group_by_module:
            ...
            for chunk in yield_module_grouped_chunks(
                ...
                get_target_dtype_fn=self._target_dtype,
                prepare_tensor_fn=lambda name, tensor, target_dtype: prepare_weight_sync_tensor(
                    self._model_type, name, tensor, target_dtype
                ),
            ):
                yield chunk
...
class FSDPPolicyWorkerBase(PolicyWorkerBase):
    ...
    def grug_validation_snapshot(self, names=()):
        """Return requested Grug weights on rank 0 and memory counters on every rank.
        ...
        return {
            ...
            "host_peak_rss_gb": resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1e6,
        }
    ...
    def init_model(self, model_path, num_training_steps: int = None):
        ...
        if getattr(model_config, "model_type", None) == GRUG_MOE_MODEL_TYPE:
            validate_grug_training_strategy(model_config.model_type, self.cfg.trainer.strategy)
        ...
        self.weight_extractor = FSDPWeightExtractor(
            ...
            moe_grouped_gemm=bool(self.cfg.trainer.policy.fsdp_config.get("moe_grouped_gemm", False)),
            fuse_weights=self.cfg.generator.fuse_weights,
        )
