...
class PolicyWorkerBase(Worker):
    ...
    def _grug_causal_lm(self):
        ...
        return causal_lm if getattr(config, "model_type", None) == GRUG_MOE_MODEL_TYPE else None

    def _begin_grug_query_bias_window(self, valid_tokens: int) -> None:
        causal_lm = self._grug_causal_lm()
        if causal_lm is None:
            self._grug_query_bias_accumulator = None
            return
        if getattr(self.strategy, "fsdp_strategy", None) != "fsdp2":
            raise ValueError("Grug policy training requires the FSDP2 strategy")
        if valid_tokens < 1:
            raise RuntimeError("Grug query bias requires at least one non-padding token per optimizer window")
        config = causal_lm.config
        candidate_count = max(1, valid_tokens * config.num_experts_per_tok // config.num_local_experts)
    ...
    def ppo_train(self, train_data: TrainingInputBatch) -> TrainingOutputBatch:
        ...
        is_grug_policy = self._grug_causal_lm() is not None
        ...
        for epoch in range(self.cfg.trainer.update_epochs_per_batch):
            ...
            for local_step, experience in enumerate(pbar):
                if is_grug_policy and local_step % accumulation_steps == 0:
                    ...
                    self._begin_grug_query_bias_window(valid_tokens)
