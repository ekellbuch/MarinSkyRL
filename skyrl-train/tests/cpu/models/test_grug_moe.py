...
def test_gradient_checkpointing_preserves_logits_and_gradients():
    ...
    candidate_count = max(
        1,
        tokens.numel() * reference.config.num_experts_per_tok // reference.config.num_local_experts,
    )
...
def test_query_bias_stays_finite_and_centered_across_optimizer_steps():
    ...
    for step in range(24):
        ...
        candidate_count = max(
            1,
            input_ids.numel() * model.config.num_experts_per_tok // model.config.num_local_experts,
        )
...
def test_router_matches_jax_lower_index_tie_rule():
    ...
    # jax.lax.top_k returns lower indices first when values compare equal.
