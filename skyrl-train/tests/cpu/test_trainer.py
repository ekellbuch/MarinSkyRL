...
class _ObservableGrugCausalLM:
    def __init__(self):
        self.config = SimpleNamespace(model_type=GRUG_MOE_MODEL_TYPE)
...
def test_grug_ppo_train_releases_consumed_microbatches():
    ...
    grug_model = MagicMock()
    grug_model.config.model_type = "grug_moe"
    grug_model.config.num_experts_per_tok = 2
    grug_model.config.num_local_experts = 4
    grug_model.config.num_hidden_layers = 1
    worker.model = MagicMock(model=grug_model)
